SELECT
  ...
  SUM(txns) over (order by date ASC) AS cum_txns,
  SUM(users) over (order by 'day'ASC) AS total_users,
  SUM(txns) over (order by 'day'ASC) AS total_txns,
  AVG(users) over (order by'day'ASC) AS avg_daily_users,
  AVG(txns) over (order by 'day'ASC) AS avg_daily_txns,
